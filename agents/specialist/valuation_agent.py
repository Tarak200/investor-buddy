...
def run(
    ...
    _query_hint = f"{company} {ticker} DCF valuation intrinsic value technicals"
    cached = get_cached_claims(key=ticker, domain="ValuationData", query_hint=_query_hint)
    ...
    cache_claims(key=ticker, domain="ValuationData", claims=claims, query_hint=_query_hint)
